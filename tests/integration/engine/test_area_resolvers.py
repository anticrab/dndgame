...
def _spec_circle(r: int) -> TargetingSpec:
    return TargetingSpec(
        ...
        shape=AreaShape.CIRCLE, radius_ft=r,
    )
...
def test_circle_resolver_cells() -> None:
    ...
    cells = reg.get(AreaShape.CIRCLE).squares(
        Square(5, 5), None, _spec_circle(10), _bf()  # 10ft → 2 клетки
    )
...
def test_circle_clamped_to_bounds() -> None:
    reg = default_area_shape_registry()
    cells = reg.get(AreaShape.CIRCLE).squares(
        Square(0, 0), None, _spec_circle(10), _bf()
    )
...
def test_line_resolver_cells() -> None:
    ...
    spec = TargetingSpec(
        kind=TargetKind.AREA, origin=OriginMode.FROM_CASTER,
        shape=AreaShape.LINE, length_ft=15,  # 3 клетки
    )
