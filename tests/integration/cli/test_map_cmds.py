...
def test_map_show_json(maps_dir: Path) -> None:
    result = runner.invoke(
        app, ["map", "show", "tiny", "--maps-dir", str(maps_dir), "--format=json"]
    )
    assert result.exit_code == 0
    data = json.loads(result.stdout)
    assert data["id"] == "tiny"
...
def test_map_show_medium_zoom(maps_dir: Path) -> None:
    ...
    result = runner.invoke(
        app, ["map", "show", "tiny", "--maps-dir", str(maps_dir), "--zoom=medium"]
    )
    assert result.exit_code == 0
...
def test_map_new_creates_empty(tmp_path: Path) -> None:
    md = tmp_path / "maps"
    result = runner.invoke(app, ["map", "new", "fresh", "--size=4x3", "--maps-dir", str(md)])
    assert result.exit_code == 0
    repo = YamlMapRepository(md)
    doc = repo.load("fresh")
...
def test_map_new_with_name_option(tmp_path: Path) -> None:
    md = tmp_path / "maps"
    result = runner.invoke(
        app, ["map", "new", "x", "--size=2x2", "--name=Custom Name", "--maps-dir", str(md)]
    )
    assert result.exit_code == 0
...
def test_map_new_bad_size_format_fails(tmp_path: Path) -> None:
    md = tmp_path / "maps"
    result = runner.invoke(app, ["map", "new", "x", "--size=invalid", "--maps-dir", str(md)])
    assert result.exit_code != 0
