...
def test_template_xp_defaults_to_zero() -> None:
    ...
    creature = build_creature_from_template(
        tmpl, instance_id=CreatureId("m1"), content=_NoContent()  # type: ignore[arg-type]
    )
...
def test_template_xp_propagates_to_creature() -> None:
    tmpl = MonsterTemplate(
        id="vet", name="Veteran", abilities=_abilities(),
        max_hp=20, armor_class=16, character_class="fighter", level=1, xp=60,
    )
    creature = build_creature_from_template(
        tmpl, instance_id=CreatureId("vet1"), content=_NoContent()  # type: ignore[arg-type]
    )
