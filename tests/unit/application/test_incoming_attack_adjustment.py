...
def test_paralyzed_grants_advantage_any_kind() -> None:
    ...
    assert _svc().incoming_attack_adjustment(t, distance_ft=30, attack_kind=AttackKind.RANGED) == (
        True,
        False,
    )
...
def test_prone_melee_point_blank_advantage() -> None:
    ...
    assert _svc().incoming_attack_adjustment(t, distance_ft=5, attack_kind=AttackKind.MELEE) == (
        True,
        False,
    )
...
def test_no_condition_no_adjustment() -> None:
    assert _svc().incoming_attack_adjustment(_c(), distance_ft=5, attack_kind=AttackKind.MELEE) == (
        False,
        False,
    )
