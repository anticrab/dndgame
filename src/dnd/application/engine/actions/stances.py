...
class DodgeAction:
    ...
    def execute(
        ...
        ctx.event_bus.publish(StanceTaken(actor_id=actor.id, stance=CombatStance.DODGING.value))
...
class DashAction:
    ...
    def __init__(self, *, economy: ActionEconomyCost = ActionEconomyCost.ACTION) -> None:
    ...
    def execute(
        ...
        ctx.event_bus.publish(StanceTaken(actor_id=actor.id, stance=CombatStance.DASHING.value))
        return ActionOutcome(
            success=True,
...
class DisengageAction:
    ...
    def execute(
        ...
        ctx.event_bus.publish(StanceTaken(actor_id=actor.id, stance=CombatStance.DISENGAGED.value))
        return ActionOutcome(
            success=True,
            consumed=self._economy,
            events_published=("stance.taken",),
